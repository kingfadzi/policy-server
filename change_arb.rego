...
########################
# Catalog bindings (for requirements section)
########################
...
convert_recipe_to_object(recipe) := obj if {
    count(recipe.allOf) > 0
    obj := {"allOf": [data.catalog.types[type_id] | type_id := recipe.allOf[_]]}
} else := obj if {
    count(recipe.anyOf) > 0
    obj := {"anyOf": [data.catalog.types[type_id] | type_id := recipe.anyOf[_]]}
}
