...
generate_policy_id(domain, mode) := policy_id {
...
include_release_id_if_applicable(scope_type) := {"id": release_id} {
scope_type == "release"
release_id := input.release.id
}
include_release_id_if_applicable(scope_type) := null {
scope_type != "release"
}

convert_recipe_to_object(recipe) := result {
...
calculate_due_date(scope_type, review_mode_key) := due_date {
...
# If release-scoped and a release window exists, choose the earlier of (deadline, window_start)
due_date := {
release_start_ns := time.parse_rfc3339_ns(input.release.window_start)
chosen := min([deadline_ns, release_start_ns])
format_ns_as_rfc3339(chosen)
} if scope_type == "release" and input.release.window_start != ""
else format_ns_as_rfc3339(deadline_ns)
}

format_ns_as_rfc3339(ns) := out { out := time.format_rfc3339_ns(ns) }
to_identifier(s) := out { out := replace(lower(s), " ", "_") }

########################################
# Final result (now includes requirements)
########################################
result := {
